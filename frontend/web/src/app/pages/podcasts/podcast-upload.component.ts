...
import { MatIconModule } from '@angular/material/icon'; // 👈 import necesario
...
@Component({
  ...
  imports: [
    ...
    MatIconModule, // 👈 añadido a los imports
  ],
...
export class PodcastUploadComponent {
  ...
  submit() {
    ...
    if (this.mode() === 'file') {
      ...
      // Validación relajada de URL http(s)
      try {
        const parsed = new URL(url);
        if (!/^https?:$/.test(parsed.protocol)) {
          this.snack.open('La URL debe empezar con http:// o https://', 'Cerrar', {
            duration: 3000,
          });
          return;
        }
